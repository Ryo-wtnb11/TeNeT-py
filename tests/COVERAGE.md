...
| 3 | `repartition` / `bend` | all 8; `bend` matrix-native since the storage-contract sweep -- it takes `lower_plan`'s route like `transpose` and `braid`, bit-identical to the block route on 6 providers x both degeneracy patterns x both bendable axes | `tests/ops/test_repartition.py`; `tests/ops/test_repartition_plan.py:51` (`UF`); `tests/symmetry/test_z2.py:330-357`; `tests/symmetry/test_su3_multiplicity.py:174-207`; `tests/symmetry/test_sun.py:350-382`; `tests/symmetry/test_product.py` (#312: `BendingCoefficients`/`DualBasis` forwarding, four products incl. `fZ2 x U1 x SU2`, each against `helpers.dense_repartition`) |
| 4 | `fuse` / `unfuse` | `{su2, u1, trivial, fz2}`; still on the block route and named in the storage contract's `OPEN` set (row 17) | `tests/ops/test_fusion.py:49` (`ALL_LEGS`; fZ2 joined in #146); dense oracles at `:245-262` |
...
| 5c | block-less operands: `tensordot`/`einsum`/`einsum_chain`/`compose`, `trace`, and the factorizations | `{trivial, u1, fz2, su2}` on numpy; `{su2}` on torch | `tests/ops/test_empty_operands.py` (dense-zeros oracle, the NumPy-float64 fallback when both operands are block-less, and the SU(2) arrangement where a couplable output takes the other operand's dtype -- over an abelian provider a couplable operand cannot rebalance the output, so the case only exists with fusion multiplicity); `tests/backends/test_torch.py::test_a_block_less_operand_takes_the_other_one_s_backend` |
...
| 8 | `flip_dual` (#142) | capability on all 8; numerically `{u1, fz2, su2, su3}` (+ Z2/product spaces); matrix-native, bit-identical to the block route on 7 providers x both degeneracy patterns x every axis subset x both `inv` | `tests/symmetry/test_flip_scalar.py:34`; `tests/ops/test_flip_dual.py`; `tests/test_map_view.py` (zero-cut list) |
| 9 | `embed` / `restrict` / `direct_sum` / `isometry` | `{trivial, u1, su2, fz2, product}`; `embed` and `restrict` matrix-native (a gather along a cached strided index map), `direct_sum` still on the block route and named as open | `tests/ops/test_embed.py:57`; `tests/ops/test_isometry.py:45-61`; `tests/test_map_view.py` (the surface enumeration and its `EXCLUDED`/`OPEN` classification) |
...
| 12 | network MPS/MPO/Env/dmrg/CTMRG | `{u1, su2, z2, su3, fz2, ProductProvider((fZ2, U1, SU2))}` (fermionic MPO/DMRG since #147; the product column since #312; CTMRG stays bosonic) | `tests/network/test_mps.py:24-32`; `tests/network/test_mpo.py`; `tests/network/test_hubbard.py`; `tests/network/test_heff2.py:110-125`; `tests/network/test_deferred.py` (the deferred instantiation boundary, `{u1, fz2}` incl. spinful Hubbard); `tests/network/test_density_matrix.py` (M61 Stage C's second decimation and `Env.heff2_families`, `{u1, fz2, su2}`); `tests/network/test_two_state_env.py` and `tests/integration/test_dmrg_excited.py` (M61 Stage D's `Env(bra=)`, `Env.project2`, `MPO.identity` and `dmrg_(orthogonal_to=)`, `{u1, fz2, su2}`); `tests/network/test_apply.py` (M49's `MPO.apply` and `MPO.variance`, `{u1, su2, fz2}` x both `from_terms` representations -- the dense oracle is what says the virtual-leg turn-around is charged by the leg and not by its `dual` flag); `tests/network/test_measure.py` (M48's `overlap`, `measure_mpo`, `correlation_function` and `expectation_profile`, `{u1, fz2}` -- the distance-`r` fermionic correlator against #147's explicit Jordan-Wigner oracle); `tests/network/test_entanglement.py` (M50's `MPS.schmidt_values`/`schmidt_sectors`/`entanglement_entropy` and `network.spectrum_sectors`/`entropy`, `{u1, su2}` -- the SU(2)-equals-U(1) entropy is what pins the `sqrt(qdim)` multiplet weight); `tests/network/test_from_entries.py` (M52's `MPO.from_entries`, `{u1, fz2}` -- the sparse `W` against `from_terms` in `to_dense` and in the DMRG energy, its block table and prepared-path dispatch, the corner channels, and the malformed-entry refusals); `tests/network/test_materialize.py` (M67's `MPO.materialize` and M68's `symbolic=` keyword, `{u1, fz2}` -- which `Env.heff2` branch a guide-shaped build runs on, instrumented on the two module globals the branch chooses between: the bare builder on the site-tensor path and `symbolic=True` on the prepared one, plus the operator identity across the two representations and the one-vector `heff2_families` fallback); `tests/network/test_mpo.py` (#312's two state-free rows: `MPO.from_terms` of one invariant k-site term **is** the operator `local_op` built, over `{fz2, fZ2 x U1, fZ2 x U1 x SU2 at both Frobenius-Schur signs, su2}` and over one- and two-internal-bond splits, and the Hubbard triangle where `t -> -t` is not a gauge -- energies on an open chain cannot see a whole-term sign, which is why this class of bug survived every row above; and #314's one-site term, `k = 1` of the invariant form, at every position of an open chain over `{u1, fz2, su2, su2 with j=0 (+) j=1, fZ2 x U1 x SU2}` -- held **bit-identical** to the rank-3 charge-leg route on the Abelian gradings where both routes exist, which is what says the twist bookkeeping of a factor that is both the first and the last is the empty one); `tests/network/test_envctm.py` and `tests/network/test_envctmc4v.py` (the two CTM lanes, `{z2, trivial, u1, su2, fz2}` -- including one `Trivial`-graded row, #243's full-C4v-versus-diagonal-mirror corner Hermiticity, where the grading is deliberately absent so the symmetry under test is the spatial one); `tests/network/test_evolution.py` (M79d's gates, bond metric and truncation, `{trivial, u1, fz2}` -- every claim against a dense closer of an `obc` `Peps`; since M82 phase 3 the closer is order-independent on a loop and the gate is exact on all four bonds of the 2x2 fZ2 patch, and since M84 the metric reaches its Gram form on all four environment-direction pairs -- `EnvCTM`'s ring closure pays the twist its fused environment wires hide, and the Gram comparison reads the metric through the closure `truncate_` uses rather than through a dense element) |
| 13 | map view | `{su2, u1, trivial}` + jax | `tests/ops/test_map.py:143-166`, `:347-364` |
| 14 | `map_diagonal` / `zip_blocks` (#232) | `{u1, fz2, fz2 Hubbard d=4, su2}` for the diagonal, which is now one `diagonal` per coupled sector and reads no block; `{trivial, u1, su2, fz2, product}` for `zip_blocks` (+ torch row below) | `tests/ops/test_map_diagonal.py:102-121` (the formed-dense oracle on all four), `:138-166` (the constructed SU(2) two-inner-line case); `tests/ops/test_blocks.py:386-447` |
...
| 17 | the storage contract (invariant 8) over the **whole public surface** | all 7 graded spaces the enumeration runs on -- trivial, Z2, U(1), fZ2, SU(2), SU(3), `ProductProvider((fZ2, U1, SU2))` -- x every public callable of `tenet.__all__`, `tenet.ops.linalg.__all__` and `SymmetricTensor`. Each is classified: measured at zero cuts, `EXCLUDED` (a boundary, or not an operation on a tensor), or `OPEN` (`fuse`/`unfuse`/`direct_sum`, still reading blocks) -- and the classification is asserted to partition the surface exactly, so a new operation cannot arrive unclassified | `tests/test_map_view.py::test_the_classification_covers_the_public_surface_exactly`, `::test_the_public_surface_never_cuts_a_block`, `::test_the_open_violations_are_still_open` |
...
- **jax eager + jit** — scattered per module, never parametrized over providers:
  ...
  sides of the trace boundary), `test_eigh_truncated.py` (#205's, plus `eigh(bond=)`
  under `jit` and under `grad` against central differences), `test_map.py:347-364`,
  `test_to_symmetry.py`,
...
- **jax grad** — `tests/backends/test_ad.py:53-59`: `{u1, su2, fz2, su3}` ×
  ...
  `test_linalg_expm_eig.py`, `tests/integration/test_vmc.py`,
  `tests/integration/test_ctmrg.py`.
...
- **b2. A jit cell per operation.** jit-ability is one property — no
  ...
  enumerable and pinned: `svd_truncated`, `select_bond` and `eigh_truncated`
  (`test_svd_truncated.py:564-586`, `test_select_bond.py`, `test_eigh_truncated.py`,
  `tests/network/test_envctmc4v.py`) and the containment-checking
...
- **c1.** Truncating network entry points under jit: `svd_truncated` decides a
  structure from singular *values* — `tests/network/test_envctmc4v.py`.
