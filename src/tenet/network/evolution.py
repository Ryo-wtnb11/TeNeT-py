...
**The one gap, named here rather than found later.** Under fermionic parity, a cluster
whose contraction closes a *loop* around the bond gives a metric that disagrees with the
Gram form it is supposed to be -- both environments, by a factor of order one, measured
against a dense oracle in ``tests/network/test_evolution.py``. On any cluster without a
loop (a row, a column) and under every grading that does not braid, the metric is exact
to machine precision. The cause sits below this module, in the swap-gate placement M79a
deferred for ``cor_*``/``edge_*`` -- this stage is the first physical caller of those six
primitives -- and ``docs/design.md``'s M79d entry carries the four measurements that
narrow it. Nothing here hides it: ``nonhermitian_part`` reports it on every bond.
...
class EnvNTU:
    ...
    *hair* -- the far neighbour closed on its other three virtual legs -- which is
    ...
    and positive up to floating point wherever the grading does not braid; ``truncate_``
    measures that rather than relying on it, and under fermionic parity the module
    docstring's gap is what it measures.
    ...
    and there the ket leads. The other reading gives a different metric under fermionic
    parity and the same one everywhere else, which is why the dense oracle decides it
