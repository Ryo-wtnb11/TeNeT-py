...
- `exp(-tau H)` suppresses every excited state relative to the ground state, so the state
...
- A step is one left-to-right pass and one right-to-left pass over the bonds — the same
  ...
  rank-`chi` approximation of the *state*, and makes the Trotter error `O(dt^3)` per step
