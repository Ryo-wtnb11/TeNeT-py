...
- `h_bond()` is six blocks, and they are the six numbers a textbook writes in the
  `{uu, ud, du, dd}` basis. The other ten entries of the 4×4 matrix change `S^z_tot` and
  so have no block to live in.
- `mpo()` is the same operator on a graded bond: `S^±` moves the bond charge by `∓2`, so
