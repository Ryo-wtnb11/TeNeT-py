...
basis states, `Σ_a m_a (2j_a + 1)`. That distinction runs through everything on this page.
...
`D=1` boundary leg fixes `S^z_tot` under U(1).
...
and therefore `Sweep(chi=...)`, bounds `Σ_c qdim(c)·m_c` — the **dense** dimension — not
the multiplet count. Under U(1) and fermionic parity the two coincide; under SU(2) they do
not.
...
`chi` grows and can undershoot by up to `max qdim(c) - 1`.
...
and under U(1) alike, while `-Σ p log p` over the flattened SU(2) spectrum would report 0.
