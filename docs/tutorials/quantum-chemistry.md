...
records with 1-based orbital indices. `k == l == 0` marks a one-body element and
`i == j == k == l == 0` the core energy:
...
The core energy is a constant shift, not a term; add it to the converged energy at the
end.
...
At two bond dimensions: a state converging on an eigenstate has a variance falling towards
zero as `chi` grows. On a small enough case, `h.materialize().to_dense()` and
`numpy.linalg.eigvalsh` give the exact answer to compare against directly.
